...
reset_configs_and_data() {
    ...
    if [[ -d "${SETUP_DIR}/secrets" || \
        ...
        read -p "Found Existing Netbird Config and Data. Do you want to continue? (y/n, default: y): " user_input
        ...
        if [[ "$user_input" == "y" || "$user_input" == "Y" ]]; then
            ...
            echo "Aborting. The config and data were not removed."
            exit 0;
        fi
    else
...
setup() {
    ...
    if [ -z "$backup_list" ]; then
        ...
        case "$choice" in
            ''|*[!0-9]*)
                warn "⚠️ Invalid selection, defaulting to 0 (skip restore)"
                choice=0
                ;;
            *)
                if [ "$choice" -gt $((count - 1)) ]; then
                    warn "⚠️ Selection out of range, defaulting to 0 (skip restore)"
                    choice=0
                fi
                ;;
        esac
