...
restore_secrets_or_generate() {
    ...
    progress "🔐 Preparing Docker Secrets..."
    # Restore secrets from backup if available
    if [[ -n "${backup_contents:-}" && $(echo "${backup_contents}" | grep -c 'secrets/') -gt 0 ]]; then
...
restore_or_init_step_ca() {
    local latest_backup="$1"
    local backup_contents="$2"
    # Restore Step CA data from backup if available
    if [[ -n "${latest_backup}" && $(echo "${backup_contents}" | grep -c 'step-ca-data/') -gt 0 ]]; then
...
generate_management_config() {
    ...
    if [[ -f "management-template.json" ]]; then
        mkdir -p management
        # Replace <DOMAIN>, <NB_AUTH_SECRET>, and <DATASTORE_KEY> with actual values
        sed -e "s|<DOMAIN>|${domain}|g" \
            -e "s|<NB_AUTH_SECRET>|${NB_AUTH_SECRET}|g" \
            -e "s|<DATASTORE_KEY>|${DATASTORE_KEY}|g" \
            management-template.json > ./management/config.json
        success "✅ management config.json generated from template"
    else
        error "management-template.json file not found!"
    fi
}
restore_management_data_if_any() {
    ...
    if [[ -n "${latest_backup}" && $(echo "${backup_contents}" | grep -c 'management/data') -gt 0 ]]; then
...
select_docker_compose_template() {
    ...
    if [[ "${1:-}" == "--prod" ]]; then
        ...
        cp docker-compose-template-prod.yml docker-compose.yml
    else
        info "Using development docker-compose template"
        cp docker-compose-template-dev.yml docker-compose.yml
    fi
}
update_docker_compose_domain() {
    ...
    if [[ -f docker-compose.yml ]]; then
        ...
        warn "⚠ docker-compose.yml not found - manual config may be needed"
    fi
}
bring_up_services() {
...
setup() {
    local domain="$1"
    show_setup_banner
    echo -e "${MAGENTA}🎯 TARGET DOMAIN: ${domain}${NC}\n"
    # Select the appropriate docker-compose template based on the deployment mode argument
    select_docker_compose_template "$2"
    # Search for the 5 most recent backups
    progress "🔍 Scanning for existing backups..."
    backup_list=$(ls -1 /backups/netbird-backup-*.tar.gz 2>/dev/null | sort -r | head -n 5)
    # Check if any backups exist
    if [ -z "$backup_list" ]; then
        info "ℹ️  No backups found, generating fresh secrets & CA"
        selected_backup=""
    else
        # Display available backups for user selection
        echo -e "${YELLOW}📦 Available backups (5 most recent):${NC}"
        count=1
        for file in $backup_list; do
            echo "  [$count] $(basename "$file")"
            count=$((count + 1))
        done
        echo "  [0] Skip restore / generate fresh secrets"
        ...
        case "$choice" in
            ''|*[!0-9]*)
                warn "⚠ Invalid selection, defaulting to 0 (skip restore)"
                choice=0
                ;;
            *)
                if [ "$choice" -gt $((count - 1)) ]; then
                    warn "⚠ Selection out of range, defaulting to 0 (skip restore)"
                    choice=0
                fi
                ;;
        esac
        ...
        if [ "$choice" -ne 0 ]; then
            ...
            info "📥 Restoring from backup: $(basename "$selected_backup")"
            backup_contents=$(tar -tzf "$selected_backup" 2>/dev/null || true)
        else
            info "ℹ️  Skipping backup restore, generating fresh secrets & CA"
        fi
    fi
    # Restore or generate secrets
    restore_secrets_or_generate "$selected_backup" "${backup_contents:-}"
    # Generate configuration files from templates
    generate_relay_env "$domain"
    generate_dashboard_env "$domain"
    generate_management_config "$domain"
    update_docker_compose_domain "$domain"
    # Initialize or restore Step CA
    restore_or_init_step_ca "$selected_backup" "${backup_contents:-}"
...
backup_ca() {
    ...
    [[ ! -d "secrets" ]] && { warn "⚠ No secrets for backup"; return 0; }
    [[ ! -d "step-ca-data" ]] && { warn "⚠ No step-ca-data for backup"; return 0; }
    [[ ! -d "management/data" ]] && { warn "⚠ No management service data for backup"; return 0; }
...
export_root_ca_if_present() {
    ...
    if [[ -f step-ca-data/certs/root_ca.crt ]]; then
        cp step-ca-data/certs/root_ca.crt /root/alpine_vm/ 2>/dev/null || true
        success "✅ Root CA certificate exported to /root/alpine_vm/"
    fi
}
